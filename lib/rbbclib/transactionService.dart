...
class TransactionService{
  ...
  Uint8List makeSignedTransaction(List<TxIn> txIns, Uint8List receiver, Account sender, int value){
    int total;

    txIns.forEach((TxIn txIn){
      UtxoOutput utxoOut = utxoTable.lookUpEntry(String.fromCharCodes(txIn.hash), txIn.index, sender.address);
...
class UserTxService{
  ...
  Uint8List makeSignedTransaction(Uint8List receiver, int value){
    ...
    userUtxoTable.utxoReturns.forEach((utxoReturn){
      txIns.add(TxIn(HEX.decode(utxoReturn.hash), utxoReturn.index, this.account.address));
    });

    List<TxOut> txOuts = List();
    txOuts.add(TxOut(value, this.account.publicKey, receiver));
    txOuts.add(TxOut(total - value, this.account.publicKey , this.account.address));
